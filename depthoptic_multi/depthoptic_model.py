...
# ================================================== #
    ...
    def build_losses(self):
        with tf.variable_scope('losses', reuse=self.reuse_variables):
            ...
            self.G_loss                   = self.depth_loss + FLAGS.optic_reg_w * self.consistency_loss
    ...
    def of_to_rgb(self, optic_flow):
        avg = tf.reduce_mean(optic_flow)
        optic_r, optic_g = tf.split(optic_flow, num_or_size_splits=2, axis=3)
        extra_b = tf.zeros(shape=[(FLAGS.batch_size/2), FLAGS.input_height, FLAGS.input_width, 1], dtype=tf.float32)
        extra_b += avg
        return tf.concat([optic_r, optic_g, extra_b], axis=3)
    
    def build_summaries(self):
        #self.gt_optic_rgb   = self.of_to_rgb(self.gt_optic)
        ...
        with tf.device('/cpu:0'):
            ...
            tf.summary.image('pre_depth2_optic',self.pre_depth2_1_optic,     collections=self.model_collection)
